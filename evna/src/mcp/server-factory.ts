/**
 * EVNA-Next MCP server factory
 *
 * Builds a fully-wired Server instance (tools + resources handlers) with NO
 * transport attached. Transport-specific entries own the hookup:
 *   - src/mcp-server.ts  → StdioServerTransport (Claude Desktop, floatctl evna remote)
 *   - src/mcp-http.ts    → StreamableHTTPServerTransport (claude.ai connector, HTTP clients)
 *
 * Streamable-HTTP creates one Server per session — everything in here must be
 * safe to construct multiple times per process. Tool instances imported from
 * tools/index.js are process-wide singletons shared across sessions (they hit
 * shared backends; same data semantics as multiple stdio clients).
 */
...
import { brainBoot, recall, activeContext, r2Sync, askEvna } from "../tools/index.js";
...
/** Build a wired EVNA-Next Server (no transport attached). */
export function createEvnaServer(): Server {
