...
async function main() {
    try {
        ...
        // Traverse up from hooks dir to find project root
        let projectDir = __dirname;
        while (projectDir !== '/' && !projectDir.endsWith('.claude/hooks')) {
            projectDir = join(projectDir, '..');
        }
        // Go up two more levels: .claude/hooks -> .claude -> project root
        projectDir = join(projectDir, '..', '..');
